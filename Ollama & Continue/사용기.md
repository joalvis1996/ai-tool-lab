...
## 3. Ollama 실행 및 테스트
모델 다운로드 완료 후 다음과 같이 메시지를 보내면 답변이 생성된다. 
![alt text](image-2.png)

## 4. IDE & Continue 연동 
Jetbrains IDE의 설정 -> plugins 로 이동하여 Continue 플러그인을 설치한다.
![alt text](image-3.png)

설치가 완료되면 IDE 왼쪽 혹은 우측 navigator에 있는 Continue 아이콘을 클릭한다. 만약 보이지 않는다면 점3개를 눌러 활성화.
![alt text](image-4.png)

아까 다운받은 모델을 선택하고 메시지를 보내면 답변이 생성된다.
![alt text](image-5.png)
![alt text](image-6.png)

## 5. 프로젝트 코드 연동
실제 코드를 하이라이트 하거나 **Ctrl + J**를 클릭하여 Continue의 context로 첨부할 수 있다.
첨부가 완료되면 query를 작성하여 질문을 한다. 
